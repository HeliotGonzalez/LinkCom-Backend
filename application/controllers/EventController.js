...
router.get('/', async (req, res) => handleError(HTTPMethodsMap.GET, res, await serviceFactory.get('events').get()));
router.get('/:id?', async (req, res) => handleError(HTTPMethodsMap.GET, res, await serviceFactory.get('events').get(buildCriteriaFrom({...req.params, ...req.query}))));
router.get('/excluding/:userID', async (req, res) => {
    ...
    return handleError(HTTPMethodsMap.GET, res, await serviceFactory.get('events').get(
...
router.put('/', async (req, res) => handleError(HTTPMethodsMap.PUT, res, serviceFactory.get('events').create(req.body)));
